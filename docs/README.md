...
5. Brief 与受控脑图先完整验证，再以同一 IndexedDB 事务锁定和保存；初始最多 12 个节点、两层。
...
13. 每位用户自行配置硅基流动 Key；不提供系统统一 Key。历史 StepFun 凭证继续加密保留，但只为
    已经激活的旧会话提供有限准备能力，不开放新的测试、保存或激活。
...
- 硅基流动的供应商延迟和账户额度仍是运行时外部变量；失败会保留本机状态并提供重试或更换配置入口；StepFun fast 已在 PR #48 生产门禁失败并由 runtime capability gate 禁用，不再作为待验证 live path；

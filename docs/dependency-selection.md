...
2026-08-29 验证结论及后续产品采样见
[高风险集成验证记录](./integration-validation.md)：现有依赖已通过 AES-256-GCM、真实 Upstash 加密
set/get/续期/delete、12 节点 Dagre LR、真实 Chromium 长英文 DOM 尺寸/换行测量、React Flow
subtree bounds/viewport 纯计算和 instance 显式 `fitView`，以及 Mermaid strict
flowchart/timeline/pie，不需要替换依赖。Mermaid timeline 的 period 不接受 `10:00` 这类时钟冒号，
使用受控 `Minute 15` label 或降级为 flowchart/table。历史两个 Provider 的最小 probe 证明现有
AI SDK 能表达各自协议，但产品门禁证明 SDK 能力不等于 role 可用。SiliconFlow
`Qwen/Qwen3.5-4B` 已在 `hkg1` 通过双语 fast 产品门禁；StepFun 最终
`step-3.7-flash + low` 在 PR #48 双语分类失败、双语展开均 0/3，因此 runtime 将其 fast
capability 关闭。StepFun 协议 fixture 与 512-token 安全下限仍可用于独立 probe，不能绕过产品
capability gate。

...
| R-P0-09 | 选中节点显示模式化三张出招卡；领域边界接受 2–4 个直接子节点，交互式 Provider 窄契约固定生成 2 个 | 产品 5、7.7；AI 7 | P0-18 | AT-060～062 | `src/features/meeting-room/node-assistance.ts`<br>`src/features/meeting-room/meeting-canvas-view.tsx`<br>`src/modules/meeting-ai/expand-node.ts`<br>`src/modules/meeting-ai/expand-node-task.ts`<br>`src/app/api/ai/expand-node/route.ts`<br>自动化证据：`node-assistance.test.ts`（浏览器实际上下文预算）、`expand-node.test.ts`、`expand-node-task.test.ts`（1,024-token、双 `{kind,title}` 候选、title 上限）、`expand-node-client.test.ts`、`meeting-canvas-browser.test.ts`、`expand-node-route.integration.test.ts`（有凭据时走真实 Route/Prompt/Schema，并分别测两家 Provider 的三次中位数）、`e2e/preparation.spec.ts`（真实 Chromium 默认 UUID、2 个可见骨架、成功持久化与刷新、失败重试、本机异常清理、取消及迟到响应） |

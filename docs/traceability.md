...
| R-P0-02 | 用户以匿名设备会话分别保存 StepFun/硅基流动配置，显式选择当前供应商，Key 不回显且单家故障不污染另一家 | 产品 7.2；ADR 0002 | P0-07、P0-08 | AT-010～019、AT-01A～01D | `src/modules/provider-config/`<br>`src/modules/api-security/provider-config-http.ts`<br>`src/modules/meeting-ai/provider-adapter.ts`<br>`src/app/api/provider-config/`<br>`src/features/provider-config/`<br>`src/app/[locale]/settings/model/`<br>`src/features/meeting-creation/meeting-creation.tsx`<br>自动化证据：`service.test.ts`（双向保存顺序、切换、并发合并、迟到 401 隔离、v1/keyring 迁移、单家失效）、`provider-config-http.test.ts`、`provider-adapter.test.ts` 与 `configured-provider-call.test.ts`（401/403 分类、凭证 revision 归因及按 Provider/model 分离的请求策略）、`provider-structured-output.test.ts`（独立 probe 请求策略）、`e2e/provider-config.spec.ts`（实际 Route/存储适配器的双凭证刷新与反复切换）、`e2e/dashboard-guide-creation.spec.ts`；首次真实分类返回未配置时打开复用配置 gate；清会议与清模型配置双向隔离 |
...
| R-P0-09 | 选中节点显示模式化三张出招卡；领域边界接受 2–4 个直接子节点，交互式 Provider 窄契约固定生成 2 个 | 产品 5、7.7；AI 7 | P0-18 | AT-060～062 | `src/features/meeting-room/node-assistance.ts`<br>`src/features/meeting-room/meeting-canvas-view.tsx`<br>`src/modules/meeting-ai/error-contract.ts`<br>`src/modules/meeting-ai/expand-node.ts`<br>`src/modules/meeting-ai/expand-node-task.ts`<br>`src/modules/meeting-ai/provider-output-failure.ts`<br>`src/app/api/ai/expand-node/route.ts`<br>自动化证据：`node-assistance.test.ts`（浏览器实际上下文预算）、`error-contract.test.ts`（错误主码与安全分型的判别式公开契约）、`expand-node.test.ts`、`expand-node-task.test.ts`（384-token 调用方预算、StepFun 512 安全下限、5 秒边界、双 `{kind,title}` 候选、title 上限）、`provider-adapter.test.ts`（不读取正文的上游拒绝/截断/内容过滤/JSON/schema 分型）、`expand-node-client.test.ts`、`meeting-canvas-browser.test.ts`（固定安全 data attribute、0 写入、可重试）、`expand-node-route.integration.test.ts`（有凭据时先验证英文/简中分类，再走真实展开 Route/Prompt/Schema；展开每家收齐 3 个安全样本并要求 3/3 成功且中位数 ≤3 秒）、`e2e/preparation.spec.ts`（真实 Chromium 默认 UUID、2 个可见骨架、成功持久化与刷新、失败重试、本机异常清理、取消及迟到响应） |

...
| R-P0-09 | 选中节点显示模式化三张出招卡，每次仅新增 2–4 个直接子节点 | 产品 5、7.7；AI 7 | P0-18 | AT-060～062 | `src/features/meeting-room/node-assistance.ts`<br>`src/features/meeting-room/meeting-canvas-view.tsx`<br>`src/modules/meeting-ai/expand-node.ts`<br>`src/modules/meeting-ai/expand-node-task.ts`<br>`src/app/api/ai/expand-node/route.ts`<br>自动化证据：`node-assistance.test.ts`、`expand-node.test.ts`、`expand-node-task.test.ts`、`expand-node-client.test.ts`、`meeting-canvas-browser.test.ts`、`expand-node-route.integration.test.ts`（有凭据时走真实 Route/Prompt/Schema）、`e2e/preparation.spec.ts`（真实 Chromium 默认 UUID、可见骨架、成功持久化与刷新、失败重试、本机异常清理、取消及迟到响应） |

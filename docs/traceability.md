...
| R-P0-16 | 结束后生成模式化 Markdown；最多 3 个确定性 Mermaid，失败有文本降级 | 产品 7.11；AI 9；UI 12 | P0-24～026 | AT-092～096 | Spike：`src/fixtures/integration-validation/mermaid-diagrams.ts`<br>`src/modules/integration-validation/mermaid-renderer.ts`<br>`src/modules/integration-validation/mermaid-renderer.test.ts`<br>部分（P0-24/P0-26 的确定性底座，不含报告 AI、组装与页面）：`src/modules/report-domain/`<br>`src/fixtures/report-domain/index.ts`<br>自动化证据：`src/modules/report-domain/fact-draft.test.ts`、`mermaid.test.ts`、`mermaid-render.test.ts`（真实 Mermaid 11.17.2 strict 渲染）、`fallback-tables.test.ts`、`localization.test.ts`<br>报告 AI 路由、Markdown 组装、报告页与端到端降级仍待实现（Issue #11，依赖 #10） |

...
| R-P0-02 | 用户以匿名设备会话分别保存 StepFun/硅基流动配置，显式选择当前供应商，Key 不回显且单家故障不污染另一家 | 产品 7.2；ADR 0002 | P0-07、P0-08 | AT-010～019、AT-01A～01D | `src/modules/provider-config/`<br>`src/modules/api-security/provider-config-http.ts`<br>`src/modules/meeting-ai/provider-adapter.ts`<br>`src/app/api/provider-config/`<br>`src/features/provider-config/`<br>`src/app/[locale]/settings/model/`<br>`src/features/meeting-creation/meeting-creation.tsx`<br>自动化证据：`service.test.ts`（双向保存顺序、切换、并发合并、迟到 401 隔离、v1/keyring 迁移、单家失效）、`provider-config-http.test.ts`、`provider-adapter.test.ts` 与 `configured-provider-call.test.ts`（401/403 分类、凭证 revision 归因及 SiliconFlow-only Non-Think 请求体）、`provider-structured-output.test.ts`（独立 probe 请求策略）、`e2e/provider-config.spec.ts`（实际 Route/存储适配器的双凭证刷新与反复切换）、`e2e/dashboard-guide-creation.spec.ts`；首次真实分类返回未配置时打开复用配置 gate；清会议与清模型配置双向隔离 |
